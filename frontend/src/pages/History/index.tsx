  ...
  useEffect(() => {
    ...
    ApiUpload.get('upload')
      ...
      .then(data => data.map((item: any) => {
        const [file, dt, hash, extensao] = item.filename.match(/\w+?_|\w+?\.|\w{1,5}/gi).map((x: string) => x.replace(/\.|_/gi, ''))
        return {
          filename: item.filename,
          file,
          dateImport: `${dt.substr(0, 4)}-${dt.substr(4, 2)}-${dt.substr(6, 2)} ${dt.substr(8, 2)}:${dt.substr(10, 2)}:${dt.substr(12, 2)}.${dt.substr(14, 3)}`,
          hash,
          extensao
        }
      }))
        ...
        <GridCol length={12}>
          {
            historyData.map((item: HistoryDataProps) => (
              <Callout type="info" title={item.file} col={12} key={item.hash}>
                <p>
                  <LinkWrapper to={`file-data/${item.filename}`} handleActiveLink={() => false}>
                    {item.filename}
                  </LinkWrapper>
                </p>
                <p>{item.dateImport}</p>
